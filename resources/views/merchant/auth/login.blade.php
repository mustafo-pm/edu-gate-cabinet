...
<div class="grid min-h-screen lg:grid-cols-2">
    ...
    <div class="relative flex items-center justify-center p-6">
        ...
        <div class="w-full max-w-sm">
            ...
            <h1 class="text-2xl font-bold">{{ __('cabinet.auth.sign_in') }}</h1>
            ...
            <form method="POST" action="{{ route('merchant.login.attempt') }}" class="mt-6 space-y-4">
                @csrf
                <div>
                    <label class="mb-1 block text-sm font-semibold" for="email">{{ __('cabinet.auth.email') }}</label>
                    <input id="email" name="email" type="email" value="{{ old('email') }}" required autofocus
                           class="eg-input" placeholder="you@institution.uz">
                </div>
                <div>
            ...
            @production
            @else
                <p class="mt-6 text-center text-xs text-eg-muted">merchant@edu-gate.uz · password</p>
            @endproduction
        </div>
    </div>
</div>
